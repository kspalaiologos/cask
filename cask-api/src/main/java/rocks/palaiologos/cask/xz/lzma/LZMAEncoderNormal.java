...
final class LZMAEncoderNormal extends LZMAEncoder {
    ...
    static int getMemoryUsage(int dictSize, int extraSizeBefore, int mf) {
        return rocks.palaiologos.cask.xz.lz.LZEncoder.getMemoryUsage(dictSize,
    ...
    int getNextSymbol() {
        ...
        for (int rep = 0; rep < LZMACoder.REPS; ++rep) {
            ...
            if (repLens[rep] < LZMACoder.MATCH_LEN_MIN) {
                repLens[rep] = 0;
                continue;
            }
        ...
        if (mainLen < LZMACoder.MATCH_LEN_MIN && curByte != matchByte
                && repLens[repBest] < LZMACoder.MATCH_LEN_MIN)
            return 1;
        ...
        if (optEnd < LZMACoder.MATCH_LEN_MIN) {
        ...
        System.arraycopy(reps, 0, opts[0].reps, 0, LZMACoder.REPS);

        // Initialize the prices for latter opts that will be used below.
        for (int i = optEnd; i >= LZMACoder.MATCH_LEN_MIN; --i)
            opts[i].reset();
        ...
        {
            int len = Math.max(repLens[0] + 1, LZMACoder.MATCH_LEN_MIN);
            if (len <= mainLen) {
                ...
                while (true) {
                    ...
                    if (price < opts[len].price)
                        opts[len].set1(price, 0, dist + LZMACoder.REPS);
    ...
    private void updateOptStateAndReps() {
        ...
        if (opts[optCur].prev1IsLiteral) {
            ...
            if (opts[optCur].hasPrev2) {
                ...
                if (opts[optCur].backPrev2 < LZMACoder.REPS)
        ...
        if (optPrev == optCur - 1) {
            ...
            System.arraycopy(opts[optPrev].reps, 0,
                             opts[optCur].reps, 0, LZMACoder.REPS);
        } else {
            int back;
            ...
            if (back < LZMACoder.REPS) {
                ...
                opts[optCur].reps[0] = back - LZMACoder.REPS;
                System.arraycopy(opts[optPrev].reps, 0,
                                 opts[optCur].reps, 1, LZMACoder.REPS - 1);
            }
        }
    }
    ...
    private void calcNormalMatchPrices(int pos, int posState, int avail,
        ...
        for (int len = startLen; ; ++len) {
            ...
            if (len2 >= LZMACoder.MATCH_LEN_MIN) {
                ...
                if (price < opts[i].price)
                    opts[i].set3(price, optCur, dist + LZMACoder.REPS, len, 0);
            }
