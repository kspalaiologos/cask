...
class BlockInputStream extends InputStream {
    ...
    private final rocks.palaiologos.cask.xz.CountingInputStream inCounted;
    ...
    public BlockInputStream(InputStream in,
            ...
            throws IOException, rocks.palaiologos.cask.xz.IndexIndicatorException {
        this.check = check;
        ...
        if (b == 0x00)
            throw new rocks.palaiologos.cask.xz.IndexIndicatorException();
        ...
        if (!DecoderUtil.isCRC32Valid(buf, 0, headerSize - 4, headerSize - 4))
            throw new rocks.palaiologos.cask.xz.CorruptedInputException("XZ Block Header is corrupt");
        ...
        for (int i = 0; i < filters.length; ++i) {
            if (filterIDs[i] == rocks.palaiologos.cask.xz.LZMA2Coder.FILTER_ID)
                filters[i] = new rocks.palaiologos.cask.xz.LZMA2Decoder(filterProps[i]);

            else if (filterIDs[i] == rocks.palaiologos.cask.xz.DeltaCoder.FILTER_ID)
                filters[i] = new rocks.palaiologos.cask.xz.DeltaDecoder(filterProps[i]);

            else if (rocks.palaiologos.cask.xz.BCJDecoder.isBCJFilterID(filterIDs[i]))
                filters[i] = new rocks.palaiologos.cask.xz.BCJDecoder(filterIDs[i], filterProps[i]);
        ...
        rocks.palaiologos.cask.xz.RawCoder.validate(filters);
        ...
        inCounted = new rocks.palaiologos.cask.xz.CountingInputStream(in);
